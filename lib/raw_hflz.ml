...
module Typing = struct
 ...
 let new_tyvar : unit -> tyvar =
  ...
  let rec unify : tyvar -> tyvar -> unit =
    fun tv1 tv2 ->
      ...
      | TvRef (_,r1), TvRef (_,r2) when phys_equal r1 r2 ->
          ...
          if Poly.(=) !r1 None && Poly.(=) !r2  None then begin
  ...
  type ty_env = (int, tyvar, Int.comparator_witness) Map.t (* id to tyvar *)

  class add_annot = object (self)
    (* for compatibility with Suzuki's implementation *)
    val mutable unbound_ints : id_env = Map.empty (module String)
    val mutable ty_env : ty_env = Map.empty (module Int)
      ...
      fun id_env rule ->
        ...
        let id_env =
          List.fold_left var_env ~init:id_env ~f:begin fun env (var,id,tv) ->
            Map.replace env ~key:var.name ~data:id
          end
        in
        Log.debug begin fun _ -> 
    ...
    method hes : hes -> unit Hflz.hes =
      fun hes ->
        let id_env =
          List.fold_left hes ~init:(Map.empty (module String)) ~f:begin fun id_env rule ->
            try
